...
snowsurvey_csv %>%
  count(Snow_cover) %>% 
  # Anything non-numeric when trying to coerce
...
When data cleaning, good idea to look at every non-numeric entry type on its own to see patterns instead of just converting all character entries to numeric. 
